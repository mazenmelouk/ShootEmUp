...
export function detectCollision(sprite1, sprite2) {
  ...
  return (
    ((sprite1X1 <= sprite2X2 && sprite1X1 >= sprite2X1) ||
...
export function detectCollisionFireBall(sprite1, sprite2) {
  var sprite1X1 = sprite1.position.x - (sprite1.anchor.x * sprite1.width) / 6;
  var sprite1X2 =
    sprite1.position.x + ((1 - sprite1.anchor.x) * sprite1.width) / 6;
  var sprite1Y1 = sprite1.position.y - (sprite1.anchor.y * sprite1.height) / 15;
  var sprite1Y2 =
    sprite1.position.y + ((1 - sprite1.anchor.y) * sprite1.height) / 15;
...
export function getTopLeft(sprite) {
...
export function getBottomRight(sprite) {
  return new PIXI.Point(
    sprite.position.x + (1 - sprite.anchor.x) * sprite.width,
    sprite.position.y + (1 - sprite.anchor.y) * sprite.height,
  );
}

export function getCenter(sprite) {
  var topLeft = getTopLeft(sprite);
  return new PIXI.Point(
    topLeft.x + 0.5 * sprite.width,
